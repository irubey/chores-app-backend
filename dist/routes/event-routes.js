 ...
 */
router.post('/', authMiddleware, rbacMiddleware(['ADMIN']), validate(createEventSchema), asyncHandler(EventController.createEvent));
/**
 ...
 * @access  Protected, Admin only
 */
router.patch('/:eventId', authMiddleware, rbacMiddleware(['ADMIN']), validate(updateEventSchema), asyncHandler(EventController.updateEvent));
/**
 * @route   DELETE /api/households/:householdId/events/:eventId
 * @desc    Delete an event from a household
 * @access  Protected, Admin only
 */
router.delete('/:eventId', authMiddleware, rbacMiddleware(['ADMIN']), asyncHandler(EventController.deleteEvent));
