 ...
 */
router.post('/', authMiddleware, rbacMiddleware(['ADMIN']), validate(createExpenseSchema), asyncHandler(ExpenseController.createExpense));
/**
 ...
 * @access  Protected, Admin only
 */
router.patch('/:expenseId', authMiddleware, rbacMiddleware(['ADMIN']), validate(updateExpenseSchema), asyncHandler(ExpenseController.updateExpense));
/**
 * @route   DELETE /api/households/:householdId/expenses/:expenseId
 * @desc    Delete an expense from a household
 * @access  Protected, Admin only
 */
router.delete('/:expenseId', authMiddleware, rbacMiddleware(['ADMIN']), asyncHandler(ExpenseController.deleteExpense));
