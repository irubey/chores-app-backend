 ...
 * @access  Protected
 */
router.post('/households', authMiddleware, validate(createHouseholdSchema), asyncHandler(UserController.createHousehold));
/**
 * @route   PATCH /api/households/:householdId
 * @desc    Update household details
 * @access  Protected, Admin only
 */
router.patch('/households/:householdId', authMiddleware, rbacMiddleware(['ADMIN']), validate(updateHouseholdSchema), asyncHandler(UserController.updateHousehold));
/**
 * @route   POST /api/households/:householdId/members
 * @desc    Add a new member to a household
 * @access  Protected, Admin only
 */
router.post('/households/:householdId/members', authMiddleware, rbacMiddleware(['ADMIN']), validate(addMemberSchema), asyncHandler(UserController.addMember));
/**
 * @route   DELETE /api/households/:householdId/members/:memberId
 * @desc    Remove a member from a household
 * @access  Protected, Admin only
 */
router.delete('/households/:householdId/members/:memberId', authMiddleware, rbacMiddleware(['ADMIN']), asyncHandler(UserController.removeMember));
/**
 * @route   DELETE /api/households/:householdId
 * @desc    Delete a household
 * @access  Protected, Admin only
 */
router.delete('/households/:householdId', authMiddleware, rbacMiddleware(['ADMIN']), asyncHandler(UserController.deleteHousehold));
